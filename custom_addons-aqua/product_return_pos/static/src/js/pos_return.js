...
var _t = core._t;
var exports = {};
var PaymentScreenWidget = pos_screens.PaymentScreenWidget;

models.load_models({
        model: 'pos.order', 
        fields: ['id', 'name', 'session_id', 'date_order', 'pos_reference', 'partner_id', 'amount_total', 'lines', 'amount_tax', 'return_ref'],
        domain: function(self){
        	if (!self.config.refund_order)
        		return [['return_ref','=',false]]; 
        	},
        loaded: function (self, pos_orders) {
            var orders = [];
            
            for (var i in pos_orders){
            	var order_date = Date.parse(pos_orders[i].date_order);
            	var today = Date.now();
            	var time_diff = today - order_date
            	var days_diff = Math.floor(time_diff / (1000 * 60 * 60 * 24));
            	var return_days = parseFloat(self.config.days_return);
            ...
            for (var i in pos_orders){
                ...
                self.order[i].amount_total = parseFloat(self.order[i].amount_total).toFixed(2);
            }
        },
...
models.load_models({
    ...
    fields: ['product_id', 'qty', 'price_unit', 'price_subtotal_incl', 'order_id', 'discount', 'returned_qty', 'name'],
...
pos_screens.define_action_button({
    ...
    'widget': ReturnButton,
    'condition': function(){
        return this.pos.config.module_pos_return;
    },
});
    ...
    _save_to_server: function (orders, options) {
        ...
        for (var i in orders) {
            ...
            new_order = {
                ...
                'lines_data': orders[i].data.lines
            };
            console.log("==new")
            console.log(self)
            console.log(new_order.lines)
            if (self.config.refund_order || new_order['return_ref'] == false )
    ...
    render_list: function(orders){
        ...
        this.$('.order-list-lines').delegate('.return-button','click',function(event){
            ...
            if (order_new.return_ref){
            	...
            	if(order_new.return_status == 'fully_return'){
            		 self.gui.show_popup('error',{
...
var OrderReturnWidget = PosBaseWidget.extend({
    ...
    for (var order in this.pos.pos_orders){
    	if(this.pos.pos_orders[order].pos_reference == this.pos_reference){
    		for (var line in this.pos.order_line){
    			var order_line = this.pos.order_line[line];
    			if ((this.pos.pos_orders[order].lines) && (this.pos.pos_orders[order].lines.includes(order_line.id))){
    				var new_vals = {}
    				var product = []
    				if (order_line.qty - order_line.returned_qty > 0){
    					for(var i in order_line.product_id){
    						product.push(order_line.product_id[i])
    						
    					}
    					new_vals = {
    	                        'product_id': product[0],
    	                        'product': product[1],
    	                        'qty': order_line.qty - order_line.returned_qty,
    			...
    			if(!(this.pos.pos_orders[order].lines)){
    				console.log(this.pos.pos_orders[order].lines_data)
