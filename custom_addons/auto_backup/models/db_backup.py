...
class db_backup(models.Model):
    ...
    #columns for dropbox
    dropbox_write = fields.Boolean('Upload to Dropbox', default=False)
    access_key = fields.Char('Access Token')
    upload_path = fields.Char("Path to upload", default='/')
    ...
    def schedule_backup(self):
        ...
        for rec in conf_ids:
            ...
            #check if user wants to upload to dropbox or not.
            if rec.dropbox_write is True:
                dbx = dropbox.Dropbox(rec.access_key)
                path = os.path.join(rec.upload_path, bkp_file)
