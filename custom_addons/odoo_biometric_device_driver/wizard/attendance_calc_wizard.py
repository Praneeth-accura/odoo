                            ...
                            if not is_record_available and not attendance.is_calculated and not is_hit:
                                if check_in_date.strftime("%w") == str(num):
                                    if check_in_date_time < float_to_time(days.hour_to):
                                        ...
                                        attendance.is_calculated = True
                                        break
                                    elif float_to_time(days.hour_from) <= check_in_date_time < float_to_time(days.hour_to) and not is_hit:
                                        vals = {
                                                'employee_id': attendance.employee_id.id,
                                                'check_in': attendance.punching_time,
                                                'date': attendance.date ,
                ...
                if check_out == False:
                    for ids in self.env['schedule.history'].search([
                        ...
                        if ids.resource_calendar_id.is_swing_shift:
                            previous_date = str((datetime.strptime(attendance.date, "%Y-%m-%d") - timedelta(days=1)).date())
                            hr_attendence_check = hr_attendance.search([('date', '<=', attendance.date), ('date', '>=', previous_date), ('employee_id', '=', attendance.employee_id.id)], order="date asc")
